...
import com.romellfudi.ussd.databinding.ActivitySplashyBinding
...
//        const val STATUS_NAV_BAR_COLOR_AS_BACKGROUND_COLOR = "status_nav_bar_color_as_background_color"
    ...
    private lateinit var binding: ActivitySplashyBinding
    override fun onCreate(savedInstanceState: Bundle?) {
        setTheme(R.style.SplashyTheme)
        super.onCreate(savedInstanceState)
        binding = ActivitySplashyBinding.inflate(layoutInflater)
    ...
    private fun setLogo() {
        ...
        if (intent.hasExtra(SHOW_LOGO)) {
            if (!intent.getBooleanExtra(SHOW_LOGO, true)) binding.ivLogo.visibility = View.GONE
        }
        ...
        if (intent.hasExtra(LOGO_WIDTH) || intent.hasExtra(LOGO_HEIGHT)) {
            ...
            val widthInDp = TypedValue.applyDimension(
                TypedValue.COMPLEX_UNIT_DIP, width.toFloat(), resources.displayMetrics
            ).toInt()
    ...
    private fun setTitle() {
        ...
        if (intent.hasExtra(SHOW_TITLE)) {
            if (!intent.getBooleanExtra(SHOW_TITLE, true)) binding.tvTitle.visibility = View.GONE
        }
        when {
            ...
            intent.hasExtra(TITLE_RESOURCE) -> {
                binding.tvTitle.text = resources.getString(
                    intent.getIntExtra(
                        TITLE_RESOURCE, applicationInfo.labelRes
                    )
                )
            }
        ...
        if (intent.hasExtra(TITLE_COLOR)) {
            binding.tvTitle.setTextColor(
                ContextCompat.getColor(
                    this, intent.getIntExtra(TITLE_COLOR, R.color.black)
                )
            )
        }
        if (intent.hasExtra(TITLE_COLOR_VALUE)) {
            binding.tvTitle.setTextColor(Color.parseColor(intent.getStringExtra(TITLE_COLOR_VALUE)))
        }
    ...
    private fun setSubTitle() {
        ...
        if (intent.hasExtra(SUBTITLE_RESOURCE)) {
            binding.tvSubTitle.text = resources.getString(
                intent.getIntExtra(
                    SUBTITLE_RESOURCE, R.string.splashy_subtitle
                )
            )
            binding.tvSubTitle.visibility = View.VISIBLE
        }
        ...
        if (intent.hasExtra(SUBTITLE_COLOR)) {
            binding.tvSubTitle.setTextColor(
                ContextCompat.getColor(
                    this, intent.getIntExtra(SUBTITLE_COLOR, R.color.eight)
                )
            )
        }
    ...
    private fun setSplashBackground() {
        if (intent.hasExtra(BACKGROUND_COLOR)) {
            binding.ivBackground.setBackgroundColor(
                ContextCompat.getColor(
                    this, intent.getIntExtra(BACKGROUND_COLOR, R.color.white)
                )
            )
        }
        ...
        if (intent.hasExtra(BACKGROUND_RESOURCE)) {
            binding.ivBackground.setBackgroundResource(
                intent.getIntExtra(
                    BACKGROUND_RESOURCE, R.color.white
                )
            )
        }
    }
                    ...
                    binding.ivLogo.animation.setAnimationListener(object :
                        ...
                        override fun onAnimationEnd(animation: Animation?) {
                            ...
                            binding.tvTitle.animation = AnimationUtils.loadAnimation(
                                this@SplashyActivity, R.anim.slide_from_logo
                            )
